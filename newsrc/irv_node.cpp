...
void IRVNode::update(const IRVBallot &b, std::vector<int> path, int count) {
  ...
  // parameter.
  int i = depth;
  while (b.preferences[i] != nextCandidate)
    ++i;
...
float IRVNode::marginalProbability(const IRVBallot &b, std::vector<int> path,
  ...
  int i = depth;
  for (i = depth; b.preferences[i] != nextCandidate; ++i)
    ++i;
  int next_idx = i - depth;
  ...
  b_beta = 0.;
  for (auto j = 0; j < nChildren + 1; ++j)
    if (j != next_idx)
      b_beta += alphas[j] + alpha0;
  branchProb = rBeta(a_beta, b_beta, engine);

  // Stop if the number of children is 2 or if there is no next preference.
  ...
  // sampling.
  if (children[next_idx] == nullptr) {
    // The number of ballot choices left is equal to the number of children
    // minus the number is -1s in the ballotPath.
    int nRemaining = nChildren;
    for (auto j = 0; j < nChildren + depth; ++j)
      nRemaining -= (b.preferences[j] == -1);

    for (auto j = nChildren - 1; j > nChildren - nRemaining; --j) {
      branchProb *= rBeta(alpha0, alpha0 * j, engine);
    }
    return branchProb;
  }
  // Otherwise recursively evaluate branch probabilities.
  std::swap(path[depth], path[i]);
