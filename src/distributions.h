 ...
 * indexArray.
 */
election *rElections(int *nBallots, int nElections, int nCandidates,
                     int *indexArray, int *permutationArray, int nChosen,
                     std::mt19937 engine) {
  BallotCount *bc;
  bool atLeastOne;
  int *nextNBallots;
  ...
  float *alpha =
      new float[nCandidates]; // Initalize alpha to uniform parameter of ones.
  for (int i = 0; i < nChosen - nCandidates; ++i) {
    alpha[i] = 1.;
  }
  if (nChosen ==
      nCandidates - 1) { // If ballot is completely specified, return.
  ...
  for (int i = 0; i < nCandidates - nChosen; ++i) {
    ...
    childBallotSets =
        rElections(nextNBallots, nElections, nCandidates, indexArray,
                   permutationArray, nChosen + 1, engine);
    std::swap(permutationArray[i + nChosen], permutationArray[nChosen]);
